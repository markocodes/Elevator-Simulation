...
public class Floor implements Runnable {
	...
	public void run() {
		...
		try {
			...
			DatagramSocket socket = new DatagramSocket(5000); // Creates a new socket. This will be used for sending and
																// recieving packets
			// socket.setSoTimeout(5000); //Sets the timeout value to 5 seconds. If 5
			// seconds elapses and no packet arrives on receive, an exception will be thrown
			...
			for (PersonRequest request : dataLines) {
				...
				DatagramPacket packetToSend = new DatagramPacket(dataArray, dataArray.length, local, 23); // Creates a
																											// packet
																											// from the
																											// dataArray,
																											// to be
																											...
																											// intermediate
																											// host
				DatagramPacket replyPacket = new DatagramPacket(new byte[20], 20); // Creates a packet to recieve the
																					// acknowledgement in.
				...
				boolean receieved = false; // defines a flag to check for receieving a actual packet vs a nothing to
											// report packet ("null")
				DatagramPacket receivedPacket = new DatagramPacket(new byte[20], 20); // Creates a new packet for
																						// receiving
...
//				}
	...
	/**
	 * Write the data obtained from the input requests file to the Controller so
	 * that it can be read by the Scheduler thread
	 * 
	 * @param dataLines is the ArrayList of PersonRequest objects that are read and
...
//	}
	...
	public static void printPacket(DatagramPacket receivedPacket, boolean sending) {
		if (!sending) { // If the packet was recieved
			System.out
					.println("Floor: Received the following packet (String): " + new String(receivedPacket.getData())); // Print
																														// data
																														// as
																														// string
																														// (Binary
			...
			System.out.println("From:" + receivedPacket.getAddress() + " on port: " + receivedPacket.getPort()); // Prints
																													// the
																													// address
																													// and
																													// port
			...
			System.out.println("Floor: Sending the following packet (String): " + new String(receivedPacket.getData()));// Print
																														// data
																														// as
																														// string
																														// (Binary
			...
			System.out.println(receivedPacket.getData()[receivedPacket.getData().length - 1]);
			System.out.println("To:" + receivedPacket.getAddress() + " on port: " + receivedPacket.getPort()); // Prints
																												// the
																												// address
																												// and
																												// port
