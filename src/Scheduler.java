...
public class Scheduler implements Runnable {
    ...
    /**
     * The Floor constructor initializes an instance of Scheduler and initializes necessary fields.
     */
        ...
        try {
            ...
            while (true) {
                if (currentState == State.WAIT_FOR_FLOOR_REQUEST) {
                    ...
                    // Find an elevator that is available to handle the new request (and all other unscheduled requests)
    ...
    /**
     * Implements the state cycle that occurs when there is communication from the elevator to the floor.
     */
            ...
            while (true) {
                ...
                if (currentState == State.WAIT_FOR_ELEVATOR_COMPLETION) {
                    ...
                    if (new String(receivedResponsePacket.getData()).trim().equals("request")) {
                        ...
                        printPacket(ackPacket, true);
                        receiveSocket.send(ackPacket);// acknowledge that packet
                        System.out.println(java.time.LocalTime.now().format(dtf) + "  Elevator " + whichQueue + " is fixed, can send requests again");
